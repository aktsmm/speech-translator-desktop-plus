...
- タイムスタンプ付きファイル名で、翻訳/書き起こしログを UTF-8 テキストとして保存。本文にも約5分ごとの日時区切りを記録
...
メイン画面の横長の状態パネルを、コンパクトな `停止` / `録音中` バッジに変更しました。録音操作、前回保存先を開くボタン、ライブノートを別ウィンドウで開くボタンを右側にまとめています。保存先ボタンは、ログが保存され、手動停止または自動終了の後処理が正常に完了した後に表示されます。
...
1. 最新の GitHub Release を開きます。
2. `SpeechTranslatorDesktopPlus-win-x64.zip` をダウンロードします。保管用に `SpeechTranslatorDesktopPlus-win-x64-1.9.3.zip` のような version 付きコピーも公開されます。
...
| 自動終了後に「停止」表示のまま開始できない | 1.9.3 で、自動終了の後処理完了時に開始・停止ボタンを更新するよう修正しました。状態ログのエラー詳細を確認し、開始ボタンが有効になったら再開してください。終了処理の失敗が表示された場合は停止を再試行できます。 |
...
保存するテキストには、最初の発話と、前回の日時記録から5分以上経過した最初の発話の直前に、PC のローカル日時と UTC オフセットを記録します。書き起こし・翻訳の両方に対応し、マイクと PC 音声で同じファイルに重複した日時を追加しません。
...
無音中には空の記録を追加せず、発話再開時の日時を残すため、長時間録音の空白を確認できます。日時は認識結果を保存した時刻であり、音声の厳密な開始時刻ではありません。この区切りは保存ファイルだけに追加され、画面やコピーしたログの形式は変わりません。既存の議事録への日時の補完は行いません。
