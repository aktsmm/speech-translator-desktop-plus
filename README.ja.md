...
- 前回利用した UI言語、話者言語、翻訳先言語、入力、利用モードを次回起動時に復元
...
5. 音声入力を選択します。
   ...
   - `PC音声（既定の再生デバイス）`
   - `マイク + PC音声`（既定）
6. 必要に応じて記録ファイル名を入力します。
   - 英数字、`-`、`_` のみ使用できます
7. `開始` を押します。
8. `ログクリア` を押すと、画面上の翻訳ログ・状態ログをクリアします。記録ファイル名が設定されている場合、次回の `開始` では前回ファイルへ追記せず、タイムスタンプ付きの新しい記録ファイルを作成します。
9. `別ウィンドウで翻訳を開く` を押すと、原文・翻訳文の最新3件だけを別ウィンドウで確認できます。

記録ファイル名が空の場合、翻訳/書き起こしは画面に表示されますがテキストファイルには保存されません。

前回利用した UI言語、利用モード、話者言語、翻訳先言語、音声入力はローカルに保存され、次回起動時に復元されます。
